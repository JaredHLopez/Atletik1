...
export default function TeamReportTable({ reports, onPenalize, onWaive, onRestore, onReject }) {
      ...
      for (const report of reports) {
        ...
        if (report.profile_image) {
          try {
            ...
            if (!profileError && profileData) {
              urls[`profile_${report.team_id}`] = profileData.publicUrl;
            } else {
              console.error('Team profile image error:', profileError);
            }
          } catch (error) {
            console.error('Error getting team profile image URL:', error);
          }
        }
        ...
        if (report.background_image) {
          try {
            ...
            if (!bgError && bgData) {
              urls[`background_${report.team_id}`] = bgData.publicUrl;
            } else {
              console.error('Team background image error:', bgError);
            }
          } catch (error) {
            console.error('Error getting team background image URL:', error);
          }
        }
      }
    ...
    <>
      <div style={{ overflowX: "auto", position: "relative" }}>
        <table style={{
          ...
          <thead>
            <tr>
              <th style={{ border: "1px solid #eee", padding: "10px 8px", background: "#fafafa", fontWeight: 600 }}>Team Name</th>
              ...
              <th style={{ border: "1px solid #eee", padding: "10px 8px", background: "#fafafa", fontWeight: 600 }}>Intro</th>
              <th style={{ border: "1px solid #eee", padding: "10px 8px", background: "#fafafa", fontWeight: 600 }}>Bio</th>
          ...
          <tbody>
            {reports.length === 0 ? (
              <tr>
                <td colSpan="12" style={{ textAlign: "center", padding: "24px", color: "#888" }}>
              ...
              reports.map((report) => (
                <tr key={report.team_id}>
                  ...
                  <td style={{ border: "1px solid #eee", padding: "8px 6px", textAlign: "center" }}>
                    {imageUrls[`profile_${report.team_id}`] ? (
                      <img
                        src={imageUrls[`profile_${report.team_id}`]}
                        alt="Team Profile"
                        style={{
                          width: 50,
                          height: 50,
                        ...
                        onClick={() => openImageModal(imageUrls[`profile_${report.team_id}`], `${report.team_name} Profile Image`)}
                      />
                    ) : (
                  ...
                  <td style={{ border: "1px solid #eee", padding: "8px 6px", textAlign: "center" }}>
                    {imageUrls[`background_${report.team_id}`] ? (
                      <img
                        src={imageUrls[`background_${report.team_id}`]}
                        alt="Team Background"
                        style={{
                          width: 70,
                          height: 40,
                        ...
                        onClick={() => openImageModal(imageUrls[`background_${report.team_id}`], `${report.team_name} Background Image`)}
                      />
                    ) : (
                      <span style={{ color: "#888", fontSize: 12 }}>No image</span>
                    )}
                    ...
                    <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
                      {(report.approval_status === "rejected" || report.approval_status === "penalized") ? (
                        <button
                          ...
                          onClick={() => onRestore(report.team_id)}
                        >
                        ...
                        <>
                          <button
                            ...
                            onClick={() => onPenalize(report.team_id)}
                          >
                          ...
                          <button
                            ...
                            onClick={() => onReject(report.team_id)}
                          >
