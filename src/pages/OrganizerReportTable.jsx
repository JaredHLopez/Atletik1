  ...
  useEffect(() => {
    async function fetchImageUrls() {
      ...
      for (const report of reports) {
        console.log('Organizer Report paths:', {
        ...
        if (report.profile_image) {
          try {
            const { data: profileData, error: profileError } = await supabase
              .storage
              .from('profile-images')
              .getPublicUrl(report.profile_image); // Use the path directly
            
            if (!profileError && profileData) {
              urls[`profile_${report.user_id}`] = profileData.publicUrl;
            } else {
              console.error('Organizer profile image error:', profileError);
            }
          } catch (error) {
            console.error('Error getting organizer profile image URL:', error);
          }
        }
        ...
        if (report.background_image) {
          try {
            const { data: bgData, error: bgError } = await supabase
              ...
              .getPublicUrl(report.background_image); // Use the path directly
            
            if (!bgError && bgData) {
              urls[`background_${report.user_id}`] = bgData.publicUrl;
            } else {
              console.error('Organizer background image error:', bgError);
            }
          } catch (error) {
            console.error('Error getting organizer background image URL:', error);
          }
        }
      }
        ...
        <table style={{
          ...
          <thead>
            <tr>
              ...
              <th style={{ border: "1px solid #eee", padding: "10px 8px", background: "#fafafa", fontWeight: 600, maxWidth: 150 }}>Suffix</th>
              <th style={{ border: "1px solid #eee", padding: "10px 8px", background: "#fafafa", fontWeight: 600, maxWidth: 200 }}>Intro</th>
              ...
              reports.map((report) => (
                <tr key={report.user_id}>
                  ...
                  <td style={{ border: "1px solid #eee", padding: "8px 6px", textAlign: "center" }}>
                    {imageUrls[`profile_${report.user_id}`] ? (
                      <img
                        src={imageUrls[`profile_${report.user_id}`]}
                        ...
                        onClick={() => openImageModal(imageUrls[`profile_${report.user_id}`], `${report.username} Profile Image`)}
                      />
                    ) : (
                  ...
                  <td style={{ border: "1px solid #eee", padding: "8px 6px", textAlign: "center" }}>
                    {imageUrls[`background_${report.user_id}`] ? (
                      <img
                        src={imageUrls[`background_${report.user_id}`]}
                        ...
                        onClick={() => openImageModal(imageUrls[`background_${report.user_id}`], `${report.username} Background Image`)}
                      />
                    ) : (
                      <span style={{ color: "#888", fontSize: 12 }}>No image</span>
                    )}
